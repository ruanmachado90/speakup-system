...
export function buildBoletimHTML({ student, turmaInfo, notas, total1, total2, totalFinal, anoAtual }) {
  ...
  const r = 90, circ = +(2 * Math.PI * r).toFixed(2);
  ...
  const tableRows = CATEGORIAS.map((cat, i) => {
    ...
    return `<tr>
      <td class="td-av"><div class="test-row"><div class="test-icon">${catIcons[cat.tipo]}</div><div class="test-info"><div class="test-name">${cat.label} Test</div><div class="test-pts">/ ${cat.max} pts</div></div></div></td>
      <td>${nota1}</td>
      <td>${nota2}</td>
  ...
  const scaleItems = [
    ['A+','95–100','#f0fdf4','#166534'],['A','90–94','#f0fdf4','#16a34a'],
    ['B+','85–89','#eff6ff','#1e40af'],['B','80–84','rgba(14,72,254,0.06)','#2563eb'],
    ['C+','75–79','rgba(252,110,31,0.07)','#c2410c'],['C','70–74','rgba(252,110,31,0.07)','#d97706'],
    ['C-','60–69','#fffbeb','#92400e'],['D','50–59','rgba(243,9,97,0.06)','#9d174d'],
    ['F','< 50','rgba(243,9,97,0.08)','#dc2626'],
  ];
...
<style>
:root{--blue:#0e48fe;--blue-06:rgba(14,72,254,.06);--blue-10:rgba(14,72,254,.10);--orange:#fc6e1f;--red:#f30961;--success:#16a34a;--muted:#6b7280;--hint:#9ca3af;--title:'Plus Jakarta Sans',sans-serif;--body:'Montserrat',sans-serif;}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
@page{size:A4 portrait;margin:12mm 14mm}
html{background:#d6dde8}
body{font-family:var(--body);background:#d6dde8;display:flex;align-items:flex-start;justify-content:center;padding:24px 16px 40px;-webkit-font-smoothing:antialiased}
...
.logo-area{display:flex;flex-direction:column;min-width:150px}
.logo-name{font-family:var(--title);font-size:24px;font-weight:800;color:var(--blue);letter-spacing:-.04em;line-height:1}
.logo-tagline{font-size:8px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:#6b7280;margin-top:4px}
...
.boletim-subtitle{font-size:9px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:#6b7280;margin-top:4px}
.header-right{text-align:right;min-width:90px}
.header-right .h-label{font-size:8px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--hint)}
...
.smeta{font-size:9.5px;color:var(--hint);margin-top:3px;font-weight:500}
...
.col-label{font-size:7.5px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--hint);margin-bottom:2px}
...
.grades-table th{font-family:var(--body);font-size:9px;font-weight:600;letter-spacing:.07em;text-transform:uppercase;color:var(--hint);padding:10px 11px;text-align:center;border-bottom:1px solid rgba(0,0,0,.07);white-space:nowrap}
.grades-table th.th-av{text-align:left;font-size:10px;color:var(--muted);font-weight:700}
.grades-table th.th-r{color:var(--blue);font-weight:700}
.grades-table tr.sub-head th{font-size:8.5px;color:var(--hint);padding:5px 11px 8px;font-weight:600;border-bottom:1px solid rgba(0,0,0,.07);background:#fafbff}
...
.test-pts{font-size:9.5px;color:var(--hint);font-weight:400;margin-top:1px}
...
.r-title{font-family:var(--body);font-size:8.5px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--blue);margin-bottom:14px;text-align:center}
...
.circle-prog{fill:none;stroke:var(--blue);stroke-width:9;stroke-linecap:round;stroke-dasharray:${circ};stroke-dashoffset:${circ};transition:stroke-dashoffset 1.4s cubic-bezier(.22,1,.36,1)}
...
.r-status{font-family:var(--body);font-size:10px;color:var(--hint);font-weight:500;margin-bottom:8px;text-align:center}
.divider2{width:100%;height:1px;background:rgba(0,0,0,.06);margin:0 0 10px}
.r-score-row{text-align:center;margin-bottom:0}
.r-score-label{font-size:8px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--blue);margin-bottom:3px}
...
.bc-title{display:flex;align-items:center;gap:7px;font-family:var(--body);font-size:8.5px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);margin-bottom:12px}
...
.crit-text{font-size:10px;color:var(--muted);line-height:1.55}
.crit-text strong{color:var(--blue);font-weight:600;font-size:10px}
...
.scale-range{font-size:9.5px;color:var(--muted);font-weight:500;white-space:nowrap}
...
.sig-role{font-size:8.5px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--hint);margin-bottom:6px}
.sig-name{font-size:10px;color:var(--muted);font-weight:500}
...
.footer span,.footer-right{font-size:9.5px;color:var(--hint);font-weight:500}
@media print{html,body{background:#fff!important;padding:0!important;margin:0!important;display:block!important}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}a{text-decoration:none}.boletim{box-shadow:none!important;border-radius:0!important;padding:0!important;width:100%!important;max-width:100%!important;margin:0!important}.circle-prog{stroke-dashoffset:${circOffset}!important}}
</style></head><body>
<div class="boletim">
<div class="page-header">
  <div class="logo-area">
    <div class="logo-name">SpeakUp</div>
    <div class="logo-tagline">English Language Academy</div>
  </div>
  <div class="title-area">
    <div class="boletim-title">Boletim Semestral</div>
    <div class="boletim-subtitle">1º e 2º Semestre &nbsp;•&nbsp; ${anoAtual}</div>
  </div>
...
<div class="main-grid">
    ...
    <table class="grades-table">
      <thead>
        ...
        <tr class="sub-head">
          ...
          <th style="border-left:1px solid rgba(0,0,0,.06)">Nota</th>
          <th style="border-left:1px solid rgba(0,0,0,.06)" colspan="2"></th>
        </tr>
      </thead>
      <tbody>
        ${tableRows}
        <tr class="total-row">
          ...
          <td style="border-left:1px solid rgba(14,72,254,.12)"><span style="font-family:var(--title);font-size:18px;font-weight:700">${fmt(total1)}</span></td>
          <td style="border-left:1px solid rgba(14,72,254,.12)"><span style="font-family:var(--title);font-size:18px;font-weight:700">${fmt(total2)}</span></td>
  ...
  <div class="result-card">
    ...
    <div class="circle-wrap">
      <svg viewBox="0 0 200 200"><circle class="circle-bg" cx="100" cy="100" r="90"/><circle class="circle-prog" id="cp" cx="100" cy="100" r="90"/></svg>
...
<div class="bottom-grid">
  <div class="bottom-card">
    ...
    <div class="crit-item"><div class="crit-icon ci-w">${catIcons.written}</div><div class="crit-text"><strong>Written Test (50 pts)</strong> — Vocabulary, Grammar, Reading e Use of English. Aplicada ao término de cada unidade.</div></div>
    <div class="crit-item"><div class="crit-icon ci-l">${catIcons.listening}</div><div class="crit-text"><strong>Listening Test (30 pts)</strong> — Compreensão de áudio: diálogos, apresentações e narrações. Aplicada ao final do semestre.</div></div>
    <div class="crit-item"><div class="crit-icon ci-s">${catIcons.speaking}</div><div class="crit-text"><strong>Speaking Test (20 pts)</strong> — Avaliação oral em dupla. Critérios: argumentação, organização, espontaneidade e desenvoltura. Aplicada ao final do semestre.</div></div>
  </div>
  <div class="bottom-card">
...
<footer class="footer">
  ...
  <div class="footer-right">Documento oficial — uso interno e externo</div>
</footer>
</div>
<script>
window.addEventListener('load',()=>{
  const c=document.getElementById('cp');
  const circ=${circ};const offset=${circOffset};
  setTimeout(()=>{c.style.strokeDashoffset=offset;},300);
});
</script>
</body></html>`;
}
